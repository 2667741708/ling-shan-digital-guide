...
| 后台鉴权服务 | 管理员初始化、PBKDF2 密码校验、Bearer token 签发、RBAC 权限依赖和管理员账号生命周期 | [ROLE_PERMISSIONS backend/app/services/auth_service.py:L22-L43](../backend/app/services/auth_service.py#L22-L43), [ensure_admin_user backend/app/services/auth_service.py:L120-L151](../backend/app/services/auth_service.py#L120-L151), [authenticate_admin backend/app/services/auth_service.py:L156-L192](../backend/app/services/auth_service.py#L156-L192), [require_admin_permission backend/app/services/auth_service.py:L199-L206](../backend/app/services/auth_service.py#L199-L206), [admin user service backend/app/services/auth_service.py:L208-L294](../backend/app/services/auth_service.py#L208-L294) |
...
| Embedding/Rerank 服务 | 默认 hash fallback，支持 OpenAI 兼容 `/embeddings` 和可选 `/rerank`，输出实际 provider/model/dimension 元数据 | [EmbeddingResult backend/app/services/embedding_service.py:L14-L27](../backend/app/services/embedding_service.py#L14-L27), [embed_text backend/app/services/embedding_service.py:L122-L131](../backend/app/services/embedding_service.py#L122-L131), [embedding_metadata backend/app/services/embedding_service.py:L134-L149](../backend/app/services/embedding_service.py#L134-L149), [rerank_hits backend/app/services/embedding_service.py:L152-L190](../backend/app/services/embedding_service.py#L152-L190) |
| 向量检索服务 | 基于 CSV/Markdown/真实资料包/后台版本文档生成数据库 chunk，统一走 pgvector 距离排序；调试导出 JSON 仅用于观察已启用 chunk | [vector_backend_name backend/app/services/vector_store.py:L66-L67](../backend/app/services/vector_store.py#L66-L67), [ensure_default_knowledge_base backend/app/services/vector_store.py:L287-L316](../backend/app/services/vector_store.py#L287-L316), [build_knowledge_base backend/app/services/vector_store.py:L456-L506](../backend/app/services/vector_store.py#L456-L506), [_retrieve_pgvector_chunks backend/app/services/vector_store.py:L508-L524](../backend/app/services/vector_store.py#L508-L524), [retrieve_context backend/app/services/vector_store.py:L578-L599](../backend/app/services/vector_store.py#L578-L599) |
...
| v1 API 兼容层 | 提供 `/api/v1` 游客、评分、AI 能力、管理后台、RBAC 和系统状态 MVP 路由 | [backend/app/api/v1.py:L39-L545](../backend/app/api/v1.py#L39-L545) |
...
| 程序 | 文件职责 | 核心符号 |
|---|---|---|
| 本地 runner | 环境检查、依赖安装、自动拉起 PostgreSQL、执行迁移、构建知识库、测试、服务启动和 Docker 烟测分发 | [migrate_database scripts/run_local.py:L117-L123](../scripts/run_local.py#L117-L123), [main scripts/run_local.py:L211-L266](../scripts/run_local.py#L211-L266), [ensure_postgres_service scripts/run_local.py:L74-L86](../scripts/run_local.py#L74-L86) |
| Alembic 迁移脚本 | 为空库执行 baseline upgrade，对已有开发库执行 stamp，避免 `create_all` 成为唯一 schema 路径 | [migrate_db scripts/migrate_db.py:L1-L91](../scripts/migrate_db.py#L1-L91), [alembic env backend/alembic/env.py:L1-L46](../backend/alembic/env.py#L1-L46), [baseline backend/alembic/versions/0001_production_baseline.py:L1-L29](../backend/alembic/versions/0001_production_baseline.py#L1-L29) |
| PostgreSQL 备份恢复脚本 | 封装本机或 Docker 容器内 `pg_dump -Fc` 和 `pg_restore`，用于生产数据备份、回滚和迁移前快照 | [postgres_backup scripts/postgres_backup.py:L1-L142](../scripts/postgres_backup.py#L1-L142) |
...
| 真实模型目录 | 授权全身数字人 GLB/VRM 模型、参考图、哈希和验证命令的放置约定 | [models README frontend/public/avatar/models/README.md:L1-L48](../frontend/public/avatar/models/README.md#L1-L48) |
| 真实模型 manifest | 主项目和外部资产包之间的固定接口 | [lingling-avatar-manifest frontend/public/avatar/models/lingling-avatar-manifest.json:L1-L33](../frontend/public/avatar/models/lingling-avatar-manifest.json#L1-L33) |
...
| Blender 全身灵灵 GLB 脚本 | 调用 MPFB/MakeHuman 基座，叠加浅青汉服、发髻、发簪、参考图 metadata 和 8 个 viseme shape keys 的 full-body GLB | [blender_generate_lingling_avatar scripts/blender_generate_lingling_avatar.py:L20-L662](../scripts/blender_generate_lingling_avatar.py#L20-L662) |
...
| 真实 3D 模型资产 | 前端默认加载的 MPFB-based full-body `lingling-realistic.glb`、MPFB base copy、source copy 和外部资产包 manifest | [models README frontend/public/avatar/models/README.md:L1-L48](../frontend/public/avatar/models/README.md#L1-L48) |
...
| 管理员账号页 | 创建后台账号、选择角色、启用/禁用账号和重置密码 | [AdminUsers frontend/src/pages/admin/AdminUsers.vue:L1-L171](../frontend/src/pages/admin/AdminUsers.vue#L1-L171) |
...
| 后台 API 客户端 | 登录、管理员账号、知识库版本流转、历史查询、评分排行、评分趋势、感受度报告、评分审核和数字人配置请求封装 | [admin api frontend/src/api/admin.ts:L3-L102](../frontend/src/api/admin.ts#L3-L102) |
...
- 修改 [embed_text backend/app/services/embedding_service.py:L122-L131](../backend/app/services/embedding_service.py#L122-L131)、[rerank_hits backend/app/services/embedding_service.py:L152-L190](../backend/app/services/embedding_service.py#L152-L190) 或 [retrieve_context backend/app/services/vector_store.py:L578-L599](../backend/app/services/vector_store.py#L578-L599) 会影响知识库命中率、PostgreSQL pgvector 查询和游客端问答依据。
- 修改 [embed_document backend/app/services/vector_store.py:L422-L454](../backend/app/services/vector_store.py#L422-L454) 会影响后台文档切片是否真正落入 `knowledge_chunk`。
- 修改 [run_local.py 命令分发 scripts/run_local.py:L211-L266](../scripts/run_local.py#L211-L266) 会影响部署、测试、数据库迁移、Docker pgvector 烟测、Docker All-in-One 烟测和服务器运行流程。
