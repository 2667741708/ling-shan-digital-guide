...
- 游客问答、RAG 检索、路线推荐、景点评分、地图导览、已内置参考 `数字人形象示例` 生成的 procedural `lingling-realistic.glb` realistic-3d 数字人渲染和 local-2d 高清 SVG 口型同步回退。
...
- 真实流式 ASR/TTS 音频链路。
- 真实多模态识图模型。
- 商业数字人 SDK、真人级绑定或 AI 图生 3D 外部高保真初模；当前已接入 Three.js/VRM `AvatarRenderer`，并内置本地 Blender 生成的 procedural `lingling-realistic.glb` 演示资产，资产优先贴近 `数字人形象示例` 的浅青古风汉服、发髻发簪、花卉/山水刺绣、玉佩流苏和表情口型方向，模型缺失时自动回退 local-2d。
