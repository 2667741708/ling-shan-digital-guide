...
LingTour AI 当前定位是比赛演示到生产加固过渡的全栈系统，重点完成景区数字人导览闭环、RAG 检索、后台权限和可复现交付，而不是千万级向量检索或实名游客用户中心。

### 当前包含

- 游客问答、RAG 检索、路线推荐、景点评分、地图导览、已内置参考 `数字人形象示例` 和 MPFB/MakeHuman 基座生成的 `lingling-realistic.glb` realistic-3d 数字人渲染和 local-2d 高清 SVG 口型同步回退。
- 管理员登录、RBAC、管理员账号维护、知识库文档维护、数字人配置、评分运营和数据大屏。
...
- 真实流式 ASR/TTS 音频链路。
- 真实多模态识图模型。
- 商业数字人 SDK、真人级骨骼绑定或 AI 图生 3D 外部高保真初模；当前已接入 Three.js/VRM `AvatarRenderer`，并内置 MPFB/MakeHuman 基座 + 本地 Blender 叠加的 `lingling-realistic.glb` 演示资产，资产优先贴近 `数字人形象示例` 的浅青古风汉服、发髻发簪、花卉/山水刺绣、玉佩流苏和表情口型方向，模型缺失时自动回退 local-2d。
...
服务层
  ├── chat_service
  ├── embedding_service / vector_store / knowledge_service
  ├── route_service
  ├── rating_service
  ├── analytics_service
  ├── auth_service
...
| v1 API 层 | 对外统一暴露 `/api/v1` 兼容接口和后台 RBAC API | [backend/app/api/v1.py:L39-L545](../backend/app/api/v1.py#L39-L545) |
...
| 知识库检索 | OpenAI 兼容 embedding/rerank fallback、PostgreSQL + pgvector chunk 写入和检索 | [embedding_service backend/app/services/embedding_service.py:L14-L190](../backend/app/services/embedding_service.py#L14-L190), [retrieve_context backend/app/services/vector_store.py:L578-L599](../backend/app/services/vector_store.py#L578-L599) |
| 后台权限 | RBAC 角色权限、管理员账号管理和权限依赖 | [ROLE_PERMISSIONS backend/app/services/auth_service.py:L22-L43](../backend/app/services/auth_service.py#L22-L43), [admin user service backend/app/services/auth_service.py:L208-L294](../backend/app/services/auth_service.py#L208-L294) |
...
| 游客评分 | 评分 upsert、情绪、统计、画像、后台筛选、评论审核、排行、趋势和感受度报告 | [create_or_update_rating backend/app/services/rating_service.py:L161-L186](../backend/app/services/rating_service.py#L161-L186), [get_admin_rating_insight_report backend/app/services/rating_service.py:L460-L546](../backend/app/services/rating_service.py#L460-L546) |
...
游客输入问题
  → /api/v1/guide/ask
  → chat_service
  → knowledge_service / vector_store / embedding_service
  → PostgreSQL knowledge_chunk.embedding pgvector 检索，可选 rerank
...
| 后台配置 | `admin_user`, `avatar_config` | RBAC 管理员与数字人配置 |
...
- PostgreSQL + pgvector 同时承担结构化数据和向量检索，减少部署复杂度；embedding/rerank 通过 OpenAI 兼容接口接入，默认保留 hash fallback。
- 游客端使用匿名 `session_uuid`，管理员端使用 Bearer token，并通过 `super_admin`、`knowledge_manager`、`operator`、`viewer` 做最小权限控制。
- 发布流程引入 Alembic baseline 和 PostgreSQL 备份恢复脚本，测试仍保留 `reset_database()` 快速建库能力。
...
- 生产加固：[TEST-027 docs/test_reference.md:L304-L337](./test_reference.md#L304-L337)、[TEST-030 docs/test_reference.md:L362-L371](./test_reference.md#L362-L371)
