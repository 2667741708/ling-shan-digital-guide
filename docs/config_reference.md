...
| `DATABASE_URL` | string | `sqlite:///data/lingtour.db` | 否 | SQLAlchemy 数据库连接；本地默认 SQLite，Docker 覆盖为 PostgreSQL | [.env.example:L14-L16](../.env.example#L14-L16), [settings backend/app/core/config.py:L7-L14](../backend/app/core/config.py#L7-L14), [configure_database backend/app/core/database.py:L33-L46](../backend/app/core/database.py#L33-L46), [deploy/docker-compose.yml:L5-L11](../deploy/docker-compose.yml#L5-L11) | 切换后会使用另一套管理员、知识文档和数字人配置；迁移前需确认数据来源 |
...
| `ADMIN_USERNAME` | string | `admin` | 否 | 启动时初始化或校验的后台管理员账号 | [.env.example:L32-L35](../.env.example#L32-L35), [ensure_admin_user backend/app/services/auth_service.py:L55-L74](../backend/app/services/auth_service.py#L55-L74) | 修改后旧账号不会自动重命名；新数据库会按新值初始化 |
| `ADMIN_PASSWORD` | string | `123456` | 否 | 后台管理员初始密码，入库前使用 PBKDF2-HMAC 哈希 | [.env.example:L32-L35](../.env.example#L32-L35), [hash_password backend/app/services/auth_service.py:L32-L41](../backend/app/services/auth_service.py#L32-L41) | 生产演示前必须修改；已存在用户不会因环境变量改变而自动重置密码 |
| `ADMIN_TOKEN_SECRET` | string | `lingtour-dev-admin-token-secret` | 否 | 后台 Bearer token HMAC 签名密钥 | [.env.example:L32-L35](../.env.example#L32-L35), [create_admin_token backend/app/services/auth_service.py:L82-L94](../backend/app/services/auth_service.py#L82-L94), [decode_admin_token backend/app/services/auth_service.py:L97-L107](../backend/app/services/auth_service.py#L97-L107) | 修改会让旧 token 立即失效；生产环境必须使用长随机值 |
